...
impl VolumesView {
    ...
    fn draw(&mut self, state: &State, frame: &mut Frame) {
        ...
        let header = Line::from(vec![
            Span::styled("Volumes", STYLES.header_style()),
            Span::styled(
...
impl BooksView {
    ...
    fn draw(&mut self, state: &State, frame: &mut Frame) {
        ...
        let line = format!("Volume {:03} - Select book", volume.number);
...
impl App {
    ...
    pub fn run(&mut self, state: &mut State) -> Result<bool> {
        self.views.clear();
        self.views.push(View::Volumes(VolumesView::default()));
        ...
        let outcome = loop {
            ...
            terminal.draw(|frame| match view {
                View::Volumes(v) => v.draw(state, frame),
            ...
            let ev = match view {
                View::Volumes(v) => v.update(key, state),
            ...
            match ev {
                ...
                ViewEvent::PopAndSelectNext => {
                    ...
                    if let Some(View::Volumes(v)) = self.views.last_mut()
                        && let Some(category) =
