...
pub fn entry(opts: &Bookvert) -> Result<()> {
    ...
    let volume_name = state.name.context("No name specified for volume")?;
...
fn config_info(opts: &Bookvert, name: &str, volume: &Volume, book: &Book) -> Result<String> {
